...
class Block {
    ...
    validate() {
        ...
        return new Promise((resolve, reject) => {
            const originalHashValue = self.hash;                    
            self.hash = null;
            const validHashValue = SHA256(JSON.stringify(self)).toString();
            self.hash = originalHashValue ;
            if(originalHashValue === validHashValue){
                resolve(true);
            } else {
                resolve(false);
            }
